...
// --- Reusable hero wrappers (same pattern as LenzHeroClient) ---

export function VisionaireHeroParallax() {
...
export function VisionaireHeroScrollIndicator() {
  const { scrollY } = useScroll();
...
export function HeroImageEntrance({
  children,
}: {
  children: React.ReactNode;
}) {
...
// --- ProductIntegrationEmbed and all sub-components ---
...
const CenterCore = () => {
  ...
  const IconComp = currentModel.icon;
    ...
    >
      ...
      <div className="relative z-10 bg-background/80 backdrop-blur-xl border border-primary/30 p-3 sm:p-4 md:p-6 rounded-lg sm:rounded-xl md:rounded-2xl shadow-[0_0_50px_rgba(var(--primary),0.15)] flex flex-col items-center justify-center text-center min-w-[100px] sm:min-w-[120px] md:min-w-[160px] lg:min-w-[180px]">
        <AnimatePresence mode="wait">
          ...
          >
            <IconComp className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 text-primary" />
          </motion.div>
        </AnimatePresence>
...
const InputsColumn = () => (
  <div className="w-full md:w-1/4 lg:min-w-[200px] z-10">
    <div className="md:hidden flex items-center gap-2 mb-2">
      <span className="text-[9px] font-mono text-primary uppercase tracking-widest">Connect Your Cameras</span>
    ...
    <div className="grid grid-cols-2 md:grid-cols-1 gap-1.5 sm:gap-2 md:gap-3 md:h-full md:justify-between">
      {inputs.map((item, i) => (
        <NodeCard key={item.id} icon={item.icon} label={item.label} subLabel={item.subLabel} badge={item.badge} side="left" delay={i * 0.1} />
      ))}
    </div>
  </div>
);
...
const OutputsColumn = () => (
  <div className="w-full md:w-1/4 lg:min-w-[200px] z-10">
    <div className="md:hidden flex items-center gap-2 mb-2">
      <span className="text-[9px] font-mono text-primary uppercase tracking-widest">Get Results</span>
      <span className="flex-1 h-px bg-primary/20" />
    </div>
    <div className="grid grid-cols-2 md:grid-cols-1 gap-1.5 sm:gap-2 md:gap-3 md:h-full md:justify-between">
      {outputs.map((item, i) => (
        <NodeCard key={item.id} icon={item.icon} label={item.label} subLabel={item.subLabel} side="right" delay={0.5 + i * 0.1} />
      ))}
    </div>
  </div>
);
...
const MobileArrow = () => (
  <div className="md:hidden flex flex-col items-center py-1">
    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="flex flex-col items-center gap-0.5">
...
const PulseLine = ({
  ...
  <>
    ...
    <motion.path
      d={d} fill="none" strokeWidth="1.5" className={color}
    ...
    />
    <motion.path
      d={d} fill="none" strokeWidth="2" className={color}
      strokeDasharray="0 1"
      initial={{ pathLength: 0, pathOffset: reverse ? 1 : 0, opacity: 0 }}
      animate={{ pathLength: [0.05, 0.2, 0.05], pathOffset: reverse ? [1, 0] : [0, 1], opacity: [0, 1, 0] }}
      transition={{ duration: 2.5 + Math.random(), repeat: Infinity, ease: "linear", delay: delay + 1.5 }}
...
const SVGConnections = () => (
  <div className="absolute inset-0 pointer-events-none hidden lg:block">
    <svg width="100%" height="100%" viewBox="0 0 1440 720" preserveAspectRatio="xMidYMid meet">
      ...
      <PulseLine d="M 720 360 C 890 360, 940 100, 1120 100" reverse delay={0.8} color="stroke-violet-500/60" />
      <PulseLine d="M 720 360 C 890 360, 940 260, 1120 260" reverse delay={1.0} color="stroke-blue-500/60" />
      <PulseLine d="M 720 360 C 890 360, 940 420, 1120 420" reverse delay={1.2} color="stroke-emerald-500/60" />
      <PulseLine d="M 720 360 C 890 360, 940 580, 1120 580" reverse delay={1.4} color="stroke-amber-500/60" />
    </svg>
  </div>
);

export function ProductIntegrationEmbed() {
  return (
