...
export default function AthenaKnowledgeMockup() {
  ...
  const sourceLabels = isBusiness
    ? ["Fiscal_Q3.pdf", "Dept_Revenue", "Risk_Protocol"]
    : ["Cam_4_Metadata", "Shift_Logs", "Lenz_Telemetry"];
  ...
  return (
    <div className="flex h-full w-full bg-white text-foreground/90 overflow-hidden font-sans text-[10px] md:text-xs select-none">
      ...
      <div className="hidden md:flex w-32 lg:w-40 bg-slate-50 border-r border-black/5 flex-col p-4 space-y-6 shrink-0">
        <div>
          <h3 className="text-[8px] lg:text-[9px] font-mono text-black/30 uppercase tracking-[0.2em] mb-4">
            Context engine
          </h3>
          <div className="space-y-2">
            {[
              {
                icon: isBusiness ? ChartBar : Database,
                label: isBusiness ? "Business" : "Vector",
              ...
              >
                <item.icon
                  className={`w-3 h-3 ${item.active ? "text-primary" : "text-black/40"}`}
                />
                <span
                  className={`font-medium text-[8px] lg:text-[9px] ${item.active ? "text-primary" : "text-black/60"}`}
                >
                  {item.label}
                </span>
              </div>
      ...
      <div className="flex-1 flex flex-col p-3 md:p-6 min-w-0 bg-white relative">
        <header className="flex justify-between items-center mb-4 md:mb-8 shrink-0">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              <span className="text-xs md:text-sm font-bold tracking-tight text-foreground uppercase tracking-widest">
                Athena AI
              </span>
              <div className="flex items-center gap-2 text-[7px] md:text-[8px] text-black/20 uppercase font-mono tracking-[0.2em]">
                <span
                  className={`w-1 h-1 rounded-full ${stage !== "idle" ? "bg-primary animate-pulse" : "bg-black/20"}`}
                />
                {stage === "idle"
                  ? "Standby"
                  : stage === "thinking"
        ...
        <div className="flex-1 flex flex-col gap-3 md:gap-5 w-full max-w-2xl mx-auto overflow-y-auto scrollbar-hide">
          <div className="flex-1 flex flex-col justify-center py-4">
            <AnimatePresence mode="wait">
              {stage === "idle" ? (
                ...
                >
                  <p className="text-xl font-light text-black/50 leading-tight">
                    Unified{" "}
                ...
                >
                  ...
                  <div className="flex gap-4">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center shrink-0 border border-black/5 shadow-sm">
                      <Brain
                        className={`w-4 h-4 ${stage !== "answering" ? "text-primary animate-pulse" : "text-primary"}`}
                      />
                    </div>
                          ...
                          >
                            <div>
                              <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-[9px] font-mono text-primary uppercase tracking-[0.2em] font-bold">
                                  Chain-of-Thought
                                </span>
                              </div>
                              <div className="space-y-2.5 pl-4 border-l-2 border-primary/10">
                                {thoughts.map((thought, i) => (
                                  <motion.div
                                    ...
                                    animate={{
                                      opacity: i <= currentThought ? 1 : 0.2,
                                      x: 0,
                                    }}
                                  ...
                                  >
                                    {i < currentThought ? (
                                      ...
                                      <div
                                        className={`w-1 h-1 rounded-full ${i === currentThought ? "bg-primary animate-pulse" : "bg-black/20"}`}
                                      />
                                    )}
                                    {thought}
                                  </motion.div>
                              ...
                              >
                                <div className="grid grid-cols-2 gap-2">
                                  {[1, 2].map((i) => (
                                    <div
                                      key={i}
                                      className="px-2.5 py-1.5 bg-white border border-black/5 rounded-lg text-[8px] flex items-center gap-2"
                                    >
                      ...
                      {stage === "answering" && (
                        ...
                        >
                          <div className="bg-slate-50 border border-black/5 p-4 rounded-2xl rounded-tl-none leading-relaxed text-black/70 text-[11px] shadow-sm relative overflow-hidden">
                            {isBusiness ? (
                              <>
                                I have synthesized your{" "}
                                <span className="text-primary font-bold">
                                  Q3 Performance Analysis
                                </span>
                                . Based on processed invoices and internal
                                fiscal documents, I've identified a{" "}
                                <span className="text-primary font-bold">
                                  12.4% increase
                                </span>{" "}
                                ...
                                <br />
                                <br />
                                Data grounded with absolute veracity across all
                                fiscal departments.
                              </>
                            ) : (
                              <>
                                ...
                                . I've identified a significant activity
                                deviation that aligns with unauthorized entry
                                protocols.
                                <br />
                                <br />
                                Live sensors and historical logs used for
                                grounding. Response verified by RAG engine.
                              </>
                            )}
                            ...
                            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-black/5">
                              {sourceLabels.map((s, i) => (
                                <div
                                  key={i}
                                  className="px-2 py-0.5 bg-primary/5 border border-primary/20 rounded text-[7px] font-bold text-primary"
                                >
                                  SOURCE: {s}
                                </div>
                              ))}
                          ...
                          <button
                            ...
                            className="text-[9px] font-mono text-black/20 hover:text-black transition-colors uppercase tracking-[0.3em] flex items-center gap-2 pt-2"
                          >
          ...
          <div className="border border-black/10 rounded-xl p-1.5 flex items-center gap-2 bg-white shadow-sm shrink-0">
            ...
            <input
              readOnly
              placeholder={query || "Ask Athena..."}
              className="flex-1 bg-transparent border-none outline-none py-2 text-xs text-foreground placeholder:text-black/10"
            />
            <button className="h-8 px-5 bg-primary text-white font-bold uppercase tracking-widest text-[9px] rounded-lg">
              Analyze
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mt-6 pt-5 border-t border-black/5 shrink-0">
          {["RAG Engine", "Context", "Reasoning", "Latency"].map((label, i) => (
            <div key={i}>
              <div className="text-[7px] font-mono text-black/20 uppercase mb-0.5 tracking-widest">
                {label}
              </div>
