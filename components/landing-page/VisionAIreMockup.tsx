    ...
    <div className="flex flex-col h-full w-full bg-white text-foreground/90 overflow-hidden font-sans text-xs select-none">
      ...
      <div className="p-4 border-b border-black/5 flex justify-between items-center bg-slate-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold tracking-tight text-foreground">VisionAIre Hub</h1>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[240px] border-r border-black/5 p-3 space-y-2.5 overflow-y-auto bg-slate-50/50">
          <h3 className="text-[9px] font-mono text-black/30 uppercase tracking-[0.2em] mb-2 px-1">Available Models</h3>
          {models.map((model) => (
            <motion.button
              key={model.id}
          ...
          <div className="pt-2 px-1">
            <div className="p-3 rounded-xl border border-dashed border-black/10 flex flex-col items-center justify-center text-black/20 hover:text-black/40 hover:border-black/20 transition-all cursor-pointer">
              ...
              <span className="text-[9px] font-mono uppercase tracking-widest text-center">Custom Training</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 p-4 flex flex-col gap-4 bg-slate-100/30">
                   ...
                   <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                      ...
                      >
                         ...
                         <div className="text-left">
                            <div className="text-[7px] font-mono text-black/40 uppercase">Registration Detected</div>
                            <div className="text-emerald-500 font-bold uppercase text-[10px]">Authorized</div>
                         </div>
                      </motion.div>
                   </div>
                 )}
                 {selectedModel.id === 'od' && (
                   <div className="absolute inset-0 p-6">
                      <div className="grid grid-cols-3 gap-3 h-full">
                        {[1,2,3,4,5,6].map(i => (
                          <div key={i} className="aspect-square border border-black/5 rounded-lg flex items-center justify-center relative overflow-hidden bg-white/40">
          ...
          <div className="grid grid-cols-3 gap-3 shrink-0">
            {[
              ...
              <div key={i} className="p-3 bg-white border border-black/5 rounded-xl shadow-sm">
