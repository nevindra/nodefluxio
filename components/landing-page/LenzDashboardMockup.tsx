...
export default function LenzDashboardMockup({
  useCase = "streams",
}: LenzDashboardMockupProps) {
    ...
    <div className="flex h-full w-full bg-white text-foreground/90 overflow-hidden font-sans text-xs select-none">
      ...
      <div className="w-16 md:w-40 bg-slate-50 border-r border-black/5 flex flex-col pt-4">
        <div className="px-4 mb-6 flex items-center gap-2">
          ...
          <span className="font-bold text-[11px] tracking-tight hidden md:block uppercase text-foreground">
            NODEFLUX
            <span className="text-primary tracking-tighter ml-0.5">OS</span>
          </span>
        </div>
        ...
        <nav className="flex-1 px-2 space-y-0.5">
          {[
            ...
            >
              <item.icon
                className={`w-3.5 h-3.5 ${item.active ? "text-primary" : "group-hover:text-black/60"}`}
              />
              <span className="hidden md:block font-medium text-[10px]">
                {item.label}
              </span>
            </div>
          ))}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white">
        ...
        <header className="h-10 bg-white border-b border-black/5 flex items-center justify-between px-4 shrink-0">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-black/5 border border-black/10 px-2 py-0.5 rounded text-[9px] flex items-center gap-2 text-foreground">
              <span>Bahasa</span>
              <CaretDown className="w-3 h-3 text-black/20" />
            </div>
          </div>
        </header>
        ...
        <div className="p-3 bg-slate-50/50 border-b border-black/5 flex flex-wrap gap-3 items-center shrink-0">
          <div className="relative group max-w-[200px] w-full">
            <MagnifyingGlass className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-black/20 group-focus-within:text-primary transition-colors" />
            <input
              readOnly
              placeholder="Search..."
          ...
          <div className="flex items-center gap-1.5">
            <div className="h-7 flex items-center gap-1 bg-white border border-black/10 px-2 rounded cursor-pointer hover:bg-slate-50 transition-colors text-[10px] text-foreground">
              <Funnel className="w-3 h-3 text-black/40" />
              <span>Semua</span>
              <CaretDown className="w-3 h-3 text-black/20" />
            </div>
            <div className="h-7 w-7 flex items-center justify-center bg-white border border-black/10 rounded cursor-pointer hover:bg-slate-50 transition-colors">
              <GridFour className="w-3 h-3 text-black/40" />
            </div>
          </div>
                ...
                {streams.map((stream) => (
                  ...
                  >
                    ...
                    <div className="absolute top-2 left-2 flex items-center gap-2">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${stream.status === "online" ? "bg-green-500" : "bg-red-500"} animate-pulse`}
                      />
                      <span className="text-[10px] font-mono font-bold tracking-tight text-white drop-shadow-md">
                        {stream.name}
                      </span>
                    </div>

                    <div className="absolute top-2 right-2 flex items-center gap-1.5">
                      <span className="bg-white/80 backdrop-blur-md border border-black/10 px-1 py-0.5 rounded text-[8px] font-mono text-foreground font-bold">
                        {stream.resolution}
                      </span>
                    </div>
                    ...
                    {stream.status === "offline" && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-white/20 backdrop-blur-[2px]">
                        ...
                        <span className="text-[10px] font-mono text-foreground/60 font-bold">
                          RECONNECTING...
                        </span>
                      </div>
                    )}
                    ...
                    <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <DotsThree className="w-4 h-4 text-white" />
                    </div>
                  </motion.div>
                ))}
              </motion.div>
              ...
              >
                <div className="grid grid-cols-2 gap-4">
                  <div className="aspect-video relative rounded-lg overflow-hidden border border-red-500/50 shadow-md">
                    ...
                    <div className="absolute top-2 left-2 bg-red-500 text-white px-2 py-0.5 rounded text-[8px] font-bold shadow-lg">
                      ALERT: UNKNOWN PERSON
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="p-4 bg-white border border-black/5 rounded-lg shadow-sm">
                      <h3 className="text-[10px] uppercase font-bold text-black/40 mb-2 tracking-wider">
                        Webhook Status
                      </h3>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <ChatCircle className="w-3 h-3 text-green-600" />
                            <span className="text-foreground">WhatsApp</span>
                          </div>
                          <span className="text-green-600 flex items-center gap-1 font-bold text-[9px]">
                            <CheckCircle className="w-2.5 h-2.5" /> Sent
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <PaperPlaneTilt className="w-3 h-3 text-blue-600" />
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-[10px] text-black/40">
                            Unknown person detected at cibiru-sore
                          </div>
                        </div>
                      </div>
                    </motion.div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-black/5 bg-slate-50">
                      <th className="px-4 py-2 font-bold uppercase tracking-wider text-[8px] text-black/40">
                        Timestamp
                      </th>
                      <th className="px-4 py-2 font-bold uppercase tracking-wider text-[8px] text-black/40">
                        Event Type
                  ...
                  <tbody>
                    {[
                      ...
                      <tr
                        key={i}
                        className="border-b border-black/5 hover:bg-slate-50/50 transition-colors"
                      >
                        <td className="px-4 py-2 font-mono text-foreground/60">
                          {row.time}
                        </td>
                        ...
                        <td className="px-4 py-2">
                          <span className="px-2 py-0.5 bg-primary/10 text-primary border border-primary/20 rounded-full text-[8px] font-bold">
                            {row.score}
                          </span>
                        </td>
                      </tr>
              ...
              >
                <div className="col-span-2 bg-white border border-black/5 rounded-lg p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-[10px] uppercase font-bold text-black/40 tracking-wider">
                      Detection Trends (24h)
                    </h3>
                    <ChartBar className="w-3.5 h-3.5 text-primary" />
                  </div>
                  <div className="h-32 flex items-end gap-2 px-2">
                    {[40, 65, 45, 90, 55, 70, 85, 40, 60, 75, 50, 80].map(
                      (h, i) => (
                        <div
                          key={i}
                          className="flex-1 bg-primary/10 hover:bg-primary transition-colors rounded-t-sm relative group"
                          style={{ height: `${h}%` }}
                        >
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div
                      key={i}
                      className="bg-white border border-black/5 rounded-lg p-3 shadow-sm"
                    >
                      <div className="text-[8px] uppercase text-black/30 font-bold mb-1 tracking-wider">
                        {stat.label}
                      </div>
        ...
        <footer className="h-8 bg-slate-50 border-t border-black/5 flex items-center justify-between px-4 shrink-0 text-[9px] font-mono text-black/40">
          <div className="flex items-center gap-4 text-foreground/60">
            ...
            <div className="flex items-center gap-1.5">
              <span>
                CPU: <span className="text-primary font-bold">42%</span>
              </span>
              <span>
                RAM: <span className="text-primary font-bold">12.4GB</span>
              </span>
            </div>
          </div>
