...
const solutionDisplayNames: Record<
  string,
...
const solutionNavItems: NavItem[] = solutions.map((solution) => ({
  ...
  desc:
    solutionDisplayNames[solution.slug]?.desc ||
    solution.description.slice(0, 50) + "...",
  longDesc:
    solutionDisplayNames[solution.slug]?.longDesc || solution.description,
}));
    ...
    items: [
      {
        title: "Lenz",
        href: "/lenz",
        desc: "Unified video management system for monitoring thousands of cameras",
      },
      {
        title: "VisionAIre",
        href: "/visionaire",
        desc: "Powerful vision AI engine with ready-to-deploy detection models",
      },
      {
        title: "Athena",
        href: "/athena",
...
export const footerNavigation = {
  ...
  solutions: navLinks[1].items.map(({ title, href }) => ({
    name: title,
    href,
  })),
  resources: [
